        ...
        if (loggedIn) {
            ...
            socket.on("update", (update) => {
                ...
                if (timeUntil > 0 && update[gameID]["started"]) {
                    ...
                    const timeUntilRatio = (timeUntil % 1000)/1000
                    if (timeUntilRatio <= 0.5){
                        setPreGameTimerOpacity(2 * timeUntilRatio**2)
                    }
                    else{
                        setPreGameTimerOpacity(2 * timeUntilRatio * (1 - timeUntilRatio) + 0.5)
                    }
